...
#
calc_loglikelihood_nbinom_weeks <- function(pars, data, squire_model, model_params,
  ...
  if (inherits(squire_model, "stochastic")) {
    ...
    pf_result <- run_deterministic_nbinom_weeks(data = data,
                                                squire_model = squire_model,
                                                model_params = model_params,
                                                model_start_date = start_date,
                                                obs_params = pars_obs,
...
#############################################################################
#############################################################################
run_deterministic_nbinom_weeks <- function(data,
                                           squire_model,
                                           model_params,
  ...
  Full_Data <- Full_Data %>%
    ...
    mutate(mu = Samples * (Mod_tot_pos_mort + rexp(length(Mod_tot_pos_mort), rate = obs_params$exp_noise))/deaths) %>%
    mutate(ll_nb = dnbinom(x = PosTestNum, size = obs_params$k_death, mu = mu, log=T)) %>%
    mutate(ll_pois = dpois(x = PosTestNum, lambda = mu, log = T))

# browser()
  # dnbinom(x = Full_Data$PosTestNum[1], size = obs_params$k_death, mu = Full_Data$mu[1], log=T)

  if(obs_params$lld =="ll_nb"){lld <- sum(Full_Data$ll_nb)}
  if(obs_params$lld =="ll_pois"){lld <- sum(Full_Data$ll_pois)}
...
# browser()
  ...
  pf_results <- list()
  pf_results$log_likelihood <- ll # + llc # + sum(lls) + sum(llp)
