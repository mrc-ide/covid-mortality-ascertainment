...
#
calc_loglikelihood_pois_bin_bin_age_weeks_decline_correction_09 <- function(pars, data, squire_model, model_params,
  ...
  if (inherits(squire_model, "stochastic")) {
    ...
    pf_result <- run_deterministic_pois_bin_bin_age_weeks_decline_correction_09(data = data,
                                                                                squire_model = squire_model,
                                                                                model_params = model_params,
                                                                                model_start_date = start_date,
                                                                                obs_params = pars_obs,
...
#############################################################################
#############################################################################
run_deterministic_pois_bin_bin_age_weeks_decline_correction_09 <- function(data,
                                                                           squire_model,
                                                                           model_params,
  ...
  ## Poisson and binomial likelihoods of deaths by age group and week
    ...
    Mod_Deaths_Age <- Mod_Deaths_Age %>% #merge(x = ., y = Comb_data) %>%
      ...
      # dplyr::filter(Week_gr != 4) %>% # Remove week 4
      mutate(Mod_cd_UTH = Mod_cd_Lus*frac_mort*ag1std, # Covid deaths in the hospital
             # Mod_ncd_UTH = Mort_deaths - Mod_cd_UTH), # Non covid deaths in the hospital
             ...
             Pos_prev = ifelse(Mod_pos_ds_UTH<=Mort_deaths,Mod_pos_ds_UTH/Mort_deaths, 0.999))

    if(obs_params$lld == "ll_pois_bin_include_4_5"){
    ...
    if(obs_params$lld == "ll_pois_bin_remove_4_5_from_pois_only"){
      # Get poisson likelihood and binomial likelihoods
      llaw <- Mod_Deaths_Age %>%
        dplyr::filter(Week_gr != 4, Week_gr != 5) %>% # Remove week 4 and 5
        mutate(ll_pois = dpois(x = Mort_deaths, lambda = (Mod_cd_Lus*frac_mort + Bg_dr)*ag1std, log = T)) %>%
        summarise(ll = sum(ll_pois)) +

        Mod_Deaths_Age %>% mutate(ll_bin = dbinom(x = PosTests, size = Samples, prob = Pos_prev, log = T)) %>%
        summarise(ll = sum(ll_bin))
      # browser()
    }

    if(obs_params$lld == "ll_pois_bin_include_4_5_remove_youngest_age_group"){
      # browser()
      llaw <- Mod_Deaths_Age %>%
        dplyr::filter(Age_gr != 1) %>%
        mutate(ll_pois = dpois(x = Mort_deaths, lambda = (Mod_cd_Lus*frac_mort + Bg_dr)*ag1std, log = T),
               ll_bin = dbinom(x = PosTests, size = Samples, prob = Pos_prev, log = T)) %>%
        summarise(ll = sum(ll_pois,ll_bin))
      # browser()
    ...
    # if(obs_params$lld == "ll_pois_bin_include_off_data"){
    #   browser()
    #
    #   llaw <- Mod_Deaths_Age %>%
    #     dplyr::filter(Age_gr != 1) %>%
