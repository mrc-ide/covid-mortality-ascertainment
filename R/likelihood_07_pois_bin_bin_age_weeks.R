...
#
calc_loglikelihood_pois_bin_bin_age_weeks_07 <- function(pars, data, squire_model, model_params,
  ...
  if (inherits(squire_model, "stochastic")) {
    ...
    pf_result <- run_deterministic_pois_bin_bin_age_weeks_07(data = data,
                                                    squire_model = squire_model,
                                                    model_params = model_params,
                                                    model_start_date = start_date,
                                                    obs_params = pars_obs,
...
#############################################################################
#############################################################################
run_deterministic_pois_bin_bin_age_weeks_07 <- function(data,
                                               squire_model,
                                               model_params,
  ...
  if(obs_params$lld == "ll_pois"){llaw <- Mod_Deaths_Age %>%
    mutate(ll_pois = dpois(x = total_deaths, lambda = Ds + Bg_dr, log = T)) %>%
    dplyr::filter(week_no != 4) %>%
  ...
  if(obs_params$lld == "ll_pois_bin"){
    ...
    # browser()
    llaw <- Mod_Deaths_Age %>%
      mutate(ll_pois = dpois(x = tot_mort_deaths, lambda = Mod_cd_UTH + Bg_dr, log = T),
             ll_bin = dbinom(x = PosTestNum, size = Samples, prob = Pos_prev, log = T)) %>%
      # filter(week_no != 4) %>%
