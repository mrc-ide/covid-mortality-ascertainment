...
#
calc_loglikelihood_hypgeo_nbinom_weeks_05 <- function(pars, data, squire_model, model_params,
  ...
  if (inherits(squire_model, "stochastic")) {
    ...
    pf_result <- run_deterministic_hypgeo_nbinom_weeks_05(data = data,
                                                squire_model = squire_model,
                                                model_params = model_params,
                                                model_start_date = start_date,
                                                obs_params = pars_obs,
...
#############################################################################
#############################################################################
run_deterministic_hypgeo_nbinom_weeks_05 <- function(data,
                                           squire_model,
                                           model_params,
  ...
  Comb_data_week <- obs_params$combined_data_week
  ...
  # Get positive deaths for the whole of Lusaka for use in nbinom

  HypGeoWeights <- obs_params$HypGeoWeights
  ...
  Full_Data <- Full_Data %>%
    ...
    dplyr::rename(tot_mort_deaths = deaths) %>%
    mutate(Mod_ncd = tot_mort_deaths - Mod_cd_Hosp) %>% # non-covid deaths
    # mutate(Mod_ncd = ifelse((tot_mort_deaths - Mod_cd_Hosp)<0,0,(tot_mort_deaths - Mod_cd_Hosp))) %>% # non-covid deaths
    mutate(Mod_pos_ncd = Mod_ncd*pcr_perc) %>%
    mutate(Mod_tot_pos_mort = Mod_cd_Hosp+Mod_pos_ncd) %>%
    mutate(Mod_tot_pos_Lus = Mod_tot_pos_mort/frac_mort)

  lld <- sum(unlist(lapply(1:nrow(Full_Data), function(x){
    browser()
    if(obs_params$lld =="ll_nb"){wtd_l <- HypGeoWeights[[x]]$weights*dnbinom(x = round(HypGeoWeights[[x]]$Pos_Deaths_Lus), size = obs_params$k_death, mu = Full_Data$Mod_tot_pos_Lus[x])}
    if(obs_params$lld =="ll_pois"){wtd_l <- HypGeoWeights[[x]]$weights*dpois(x = round(HypGeoWeights[[x]]$Pos_Deaths_Lus), lambda = Full_Data$Mod_tot_pos_Lus[x])}
    log(sum(wtd_l))
  })))
  ...
  ################################################
  ################################################
  ## In addition, we want the ll for the age distribution
  ## I suppose for each of these age groups in each week, I should estimate the pcr prevalence, then add on coincidental deaths...
  lla <- 0
  if(!is.null(obs_params$combined_data)){
  ...
  if(all(unlist(lapply(list(obs_params$comb_df, obs_params$comb_det),function(x){!is.null(x)})))){
    ...
    # Sus <- rowSums(out[,index$S])
    # infs <- as.integer(c(0,diff(max(Sus)-Sus)))
