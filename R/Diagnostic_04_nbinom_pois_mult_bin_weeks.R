...
Diagnostic_Plot <- function(fit_num, fit_Model_list, IFRvals, ll_type){
  ...
  WeekData <- readRDS("~/Documents/Imperial/PostDoc/Zambia/covid-mortality-ascertainment/analysis/data/Code-generated-data/00_13_Combined_mortuary_postmortem_data_complete.rds") %>%
    group_by(date) %>%
...
browser()
  Mod_Weekly_Deaths_Lus <- apply(fit_Model$output,3,function(x){
    ...
    # browser()
    # if(ll_type == "ll_bin"){ll <- sum(dpois(x = WeekData$PosTestNum, lambda = Pos_Ds_Lus*0.8*WeekData$Samples/WeekData$deaths, log=T))}
    if(ll_type == "ll_pois"){ll <- sum(dpois(x = WeekData$PosTestNum, lambda = Pos_Ds_Lus*0.8*WeekData$Samples/WeekData$deaths, log=T))}
    if(ll_type == "ll_nb"){ll <- sum(dnbinom(x = WeekData$PosTestNum, size = k_death, mu = Pos_Ds_Lus*0.8*WeekData$Samples/WeekData$deaths, log=T))}
