Diagnostic_Plot_09 <- function(fit_num, fit_Model_list, IFRvals, Deaths_by_week = T, Deaths_by_Age = T, Mulenga_Prev=T){
  ...
  if(fit_Model$pmcmc_results$inputs$pars_obs$lld =="ll_pois_bin_include_4_5"){
    Mod_Age_Deaths_Lus_Av <- Mod_Age_Deaths_Lus_Av %>%
      mutate(ll_bin = dbinom(x = PosTests, size = Samples, prob = Pos_prev),
             ll_pois = dpois(x = Mort_deaths, lambda = Mod_tot_ds_Mort))
  }

  if(fit_Model$pmcmc_results$inputs$pars_obs$lld =="ll_pois_bin_remove_4_5"){
    Mod_Age_Deaths_Lus_Av <- Mod_Age_Deaths_Lus_Av %>%
      mutate(ll_bin = ifelse(Week_gr %in% c(4,5), 1, dbinom(x = PosTests, size = Samples, prob = Pos_prev)),
             ll_pois = ifelse(Week_gr %in% c(4,5), 1, dpois(x = Mort_deaths, lambda = Mod_tot_ds_Mort)))
  }
  ...
  if(fit_Model$pmcmc_results$inputs$pars_obs$lld =="ll_pois_bin_include_4_5_remove_youngest_age_group"){
    Mod_Age_Deaths_Lus_Av <- Mod_Age_Deaths_Lus_Av %>%
      mutate(ll_bin = ifelse(Age_gr %in% c(1), 1, dbinom(x = PosTests, size = Samples, prob = Pos_prev)),
             ll_pois = ifelse(Age_gr %in% c(1), 1, dpois(x = Mort_deaths, lambda = Mod_tot_ds_Mort)))
  }
...
# browser()
  ...
  Mod_Age_Deaths_Lus_Av <- Mod_Age_Deaths_Lus_Av %>%
    ...
    # select(ll_bin_mean) %>% mutate(ll_bin_mean = log(ll_bin_mean))
    # ungroup() %>%
...
#   Plot_data = Mod_Age_Deaths_Lus_Av_Week %>% filter(Week_gr != 4, Week_gr != 5)} else {Plot_data = Mod_Age_Deaths_Lus_Av_Week}
  ...
  p1 <- ggplot(Mod_Age_Deaths_Lus_Av_Week, aes(x = date)) +
    ...
    ggtitle(paste("COVID-19 weekly deaths\n(UTH Mortuary) \nbin ll = ", round(sum(Mod_Age_Deaths_Lus_Av$ll_bin_mean, na.rm = T),1))) +
...
# browser()
  ...
  lls <- list(
    ll_bin = round(sum(Mod_Age_Deaths_Lus_Av$ll_bin_mean, na.rm = T),1),
    ll_pois = round(sum(Mod_Age_Deaths_Lus_Av$ll_pois_mean, na.rm = T),1),
