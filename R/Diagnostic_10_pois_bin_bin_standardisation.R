Diagnostic_Plot_09 <- function(fit_num, fit_Model_list, IFRvals, Deaths_by_week = T, Deaths_by_Age = T, Mulenga_Prev=T){
  ...
  if(fit_Model$pmcmc_results$inputs$pars_obs$lld =="ll_pois_bin_remove_4_5"){
    Mod_Age_Deaths_Lus_Av <- Mod_Age_Deaths_Lus_Av %>%
      mutate(ll_bin = ifelse(Week_gr %in% c(4,5), 1, dbinom(x = PosTests, size = Samples, prob = Pos_prev, log = T)),
             ll_pois = ifelse(Week_gr %in% c(4,5), 1, dpois(x = Mort_deaths, lambda = Mod_tot_ds_Mort, log = T)))
  }
  ...
  if(fit_Model$pmcmc_results$inputs$pars_obs$lld =="ll_pois_bin_include_4_5_remove_youngest_age_group"){
    Mod_Age_Deaths_Lus_Av <- Mod_Age_Deaths_Lus_Av %>%
      mutate(ll_bin = ifelse(Age_gr %in% c(1), 1, dbinom(x = PosTests, size = Samples, prob = Pos_prev, log = T)),
             ll_pois = ifelse(Age_gr %in% c(1), 1, dpois(x = Mort_deaths, lambda = Mod_tot_ds_Mort, log = T)))
  }
