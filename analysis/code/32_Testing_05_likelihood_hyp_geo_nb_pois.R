...
HypGeo_Weights_Weeks <- lapply(1:nrow(Comb_weeks), function(x){
  # browser()
  data.frame(Pos_Deaths_Sample = Comb_weeks$PosTestNum[x],
             Pos_Deaths_Mort = 0:Comb_weeks$deaths[x],
             Neg_Deaths_Mort = Comb_weeks$deaths[x]:0,
             Samples = Comb_weeks$Samples[x]) %>%
    mutate(Pos_Deaths_Lus = Pos_Deaths_Mort / 0.8,
           hg_l = dhyper(x = Comb_weeks$PosTestNum[x], m = Pos_Deaths_Mort, n = Neg_Deaths_Mort, k = Samples),
...
Test <- cma::fit_spline_rt(prob_non_severe_death_treatment = prob_non_severe_death_treatment$X1,
                           data = data,
                           ...
                           dur_R = Inf,
                           log_likelihood = calc_loglikelihood_hypgeo_nbinom_weeks_05,
                           lld = "ll_nb",
...
Diagnostic_Plot(1, fit_Model_list = list(Test), IFRvals = readRDS(file = "analysis/data/Code-generated-data/00_03_IFR_matrix_coefficients.rds"), ll_type = "ll_nb")
