#################################################################################################
### 3. Vary IFR and slope over model fitted to BMJ data and lancet paper, holdling rf at 100% ###
#################################################################################################
rm(list = ls())
devtools::load_all(".")
library(squire)
...
# IFR_Coefs <- lm(log(Braz_IFR[-1,1]) ~ seq(7.5, 92.5, by = 5))$coefficients

# function to calculate IFR from Intercept and Slope
IFR_calc <- function(Intercept, Slope){
  sum(exp(Age_groups * Slope + Intercept) * pop_st_lu_prov/sum(pop_st_lu_prov))
}

# function to calculate Intercept from Slope and IFR
Int_calc <- function(IFR, Slope){
  # Over a range of intercept values
  Intercept_vals <- seq(-10,-1,by = 0.00001)
  varyInt <- abs(sapply(X = Intercept_vals, FUN = IFR_calc, Slope = Slope) - IFR)
...
# IFR vector
IFR_vec <- seq(0.2,1.8, by = 0.2)
# Slope vector
Slope_vec <- seq(0.2, 1.8,by = 0.2)# * IFR_Coefs[2]
# Expand IFR/slope
IFR_mat <- expand.grid("IFR_x" = IFR_vec, "Slope_x" = Slope_vec)
IFR_mat <- IFR_mat %>% mutate(IFR_abs = IFR_x * sum(IFR_Age * pop_st_lu_prov/sum(pop_st_lu_prov)),
                              Slope_abs = Slope_x * IFR_Coefs[2])

# Apply the function to calculate Int values for each IFR gradient combination
# IFR_mat$Int_abs <- apply(IFR_mat, 1, function(x){
#   Int_calc(x["IFR_abs"],x["Slope_abs"])
# })
# saveRDS(IFR_mat, "analysis/data/Code-generated-data/IFR_mat_Ints2.rds")
...
BMJ_data_HighEst <- readRDS("analysis/data/Code-generated-data/00_05c_BMJ_Data_DailyEsts_HighEsts.rds") %>%
  select(date, Est_Deaths_BMJ_Sam_HE_Strict) %>%
...
fit_l_IFR_slope <- lapply(1:nrow(IFR_Age_var_slope_int_fil), function(x){
  ...
  prob_death_tot <- IFR_Age_var_slope_int_fil[x,]/(100*parameters_explicit_SEEIR("Zambia")$prob_hosp)
...
sero_pcr_df_l_IFR_slope <- lapply(X = fit_l_IFR_slope, FUN = function(x){
  seroprev_df(x)})
...
pdf(file = paste0("analysis/figures/12_1_BMJ_Lancet_var_IFR_slope_",Dataset_Name,".pdf"), height = 30, width = 30)
cowplot::plot_grid(plotlist = PlotsList_1, nrow = 9,byrow = F)
dev.off()

pdf(file = paste0("analysis/figures/12_2_BMJ_Lancet_var_IFR_slope_",Dataset_Name,".pdf"), height = 30, width = 30)
cowplot::plot_grid(plotlist = PlotsList_2, nrow = 9,byrow = F)
dev.off()

pdf(file = paste0("analysis/figures/12_3_BMJ_Lancet_var_IFR_slope_",Dataset_Name,".pdf"), height = 30, width = 30)
...
Likelihoods_IFR_Age_structure<- sapply(1:length(fit_l_IFR_slope), function(y){
...
dd_l_IFR_slope <- sero_pcr_df_l_IFR_slope_com %>% filter(date %in% as.Date(c("2020-07-15","2020-07-29"))) %>%
  select(replicate, date, sero_perc, pcr_perc, IFR_val, Slope_val) %>%
...
pdf(file = paste0("analysis/figures/12_5_BMJ_Lancet_var_IFR_slope_",Dataset_Name,".pdf"), height = 6, width = 20)
cowplot::plot_grid(
