...
fit_1 <- fit_spline_rt(data = data,
                       ...
                       sero_df_pos = as.numeric(as.integer(c(0.021*2614))),#, 0.106*1952))), # See Table 2
                       sero_df_samples = c(2614),#,1952),
                       pcr_df_start = as.Date(c("2020-07-04")),
                       pcr_df_end = as.Date(c("2020-07-27")),
                       pcr_df_pos = as.integer(c(0.076*2848)), # See Table 2
                       pcr_df_samples = c(2848)
)
...
# saveRDS(fit_1, file = "../Bonus Files/p_01_Official_Data_Lancet_est_rf_resonable.rds")
...
pdf(file = "analysis/figures/p_1_Official_Reported_Deaths_Lusaka_Fit_est_rf_Mulenga_Data.pdf", height = 12, width = 4)
