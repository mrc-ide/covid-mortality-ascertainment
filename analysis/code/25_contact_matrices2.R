...
# Take the data and...
df_ImpList_Count <-  lapply(Imp, function(x){
  df_ImpList[[x]] %>% group_by(site, studyDay) %>% # group by site and day
    select(par_age_gr, con_age_gr, site, studyDay) %>% count(par_age_gr, con_age_gr) %>%
    split(list(.$studyDay,.$site))}) # count numbers in each group
...
df_ImpList_Smooth <- lapply(Imp, function(x){
  lapply(1:length(df_ImpList_Count[[x]]), function(y){
    # browser()
    tmp <- df_ImpList_Count[[x]][[y]] %>%
      complete(par_age_gr, con_age_gr, fill = list(n = 0)) %>%
      mutate(ni = rep(ni_s_d[,c("ni_s2_d1","ni_s2_d2","ni_s1_d1","ni_s1_d2")][,y], each = 16),
             par_age_gr = as.numeric(par_age_gr),
             con_age_gr = as.numeric(con_age_gr),
             ni = log(as.numeric(ni)))
    # browser()
    Mod_tmp <- gam(n ~  te(par_age_gr, con_age_gr, bs="tp", k = 10) + offset(ni),
        data = tmp,
        family = nb)
    # browser()
    tmp$fitted <- predict(object = Mod_tmp, type='response')
    # browser()
    tmp <- tmp %>% ungroup() %>% select(par_age_gr, con_age_gr, fitted) %>%# count(par_age_gr, con_age_gr) %>% # count numbers in each group
      spread(key = con_age_gr, value = fitted) # create matrix
...
# as.matrix(df_ImpList_Smooth[[1]][[1]])


###################################
### 3. Construct Contact Matrix ###
###################################
...
MatList <- lapply(Imp, function(x){
  lapply(1:length(df_ImpList_Smooth[[x]]), function(y){
    df_ImpList_Smooth[[x]][[y]] %>% #select(-site,-studyDay) %>% # Remove site and studyDay columns
...
m_ij_sd <- lapply(Imp, function(x){
  lapply(1:length(MatList[[x]]), function(y){
    MatList[[x]][[y]]/(ni_s_d[,c("ni_s2_d1","ni_s2_d2","ni_s1_d1","ni_s1_d2")][,y])})
})
# MatList[[1]][[1]][1,]/(ni_s_d[,c("ni_s2_d1","ni_s2_d2","ni_s1_d1","ni_s1_d2")][1,1])
# m_ij_sd[[1]][[1]][1,]
...
m_ij_s <- lapply(Imp, function(x){
  lapply(c(1,3),function(y){
    (m_ij_sd[[x]][[y]]*ni_s_d[,c("ni_s2_d1","ni_s2_d2","ni_s1_d1","ni_s1_d2")][,y] + m_ij_sd[[x]][[y+1]]*ni_s_d[,c("ni_s2_d1","ni_s2_d2","ni_s1_d1","ni_s1_d2")][,y+1])/
      (rowSums(ni_s_d[,c("ni_s2_d1","ni_s2_d2","ni_s1_d1","ni_s1_d2")][,c(y,y+1)]))})
})
# m_ij_sd[[1]][[1]][1,]*ni_s_d[,c("ni_s2_d1","ni_s2_d2","ni_s1_d1","ni_s1_d2")][1,1]
# (m_ij_sd[[1]][[1]]*ni_s_d[,c("ni_s2_d1","ni_s2_d2","ni_s1_d1","ni_s1_d2")][,1])[1,]
...
m_ij <- lapply(Imp, function(x){
  (m_ij_s[[x]][[1]]*rowSums(ni_s_d[,c("ni_s1_d1","ni_s1_d2")]) + m_ij_sd[[x]][[2]]*rowSums(ni_s_d[,c("ni_s2_d1","ni_s2_d2")]))/
      (rowSums(ni_s_d[,c("ni_s2_d1","ni_s2_d2","ni_s1_d1","ni_s1_d2")]))
  })
...
n_j <- table(unique(df_ImpList[[1]][,c("part_id","par_age_gr")])$par_age_gr)
# n_j <- table(unique(df_ImpList[[1]][,c("cont_id","con_age_gr")])$con_age_gr) #n_j is specific to the imputation
n_i_plus_n_j <- matrix(rep(n_i, length(n_i)), nrow = length(n_i)) + t(matrix(rep(n_j, length(n_j)), nrow = length(n_j)))

Cs_ij <- lapply(Imp, function(x){
  # browser()
  # n_j <- table(unique(df_ImpList[[x]][,c("cont_id","con_age_gr")])$con_age_gr)
  # n_i_plus_n_j <- matrix(rep(n_i, length(n_i)), nrow = length(n_i)) + t(matrix(rep(n_j, length(n_j)), nrow = length(n_j)))
  (C_ij[[x]]*c(n_i) + t(C_ij[[x]]*c(n_j)))/ n_i_plus_n_j
})
...
heatmap(t(cs_ij_av*pop_i),Rowv = NA, Colv = NA)

heatmap(t(squire::get_mixing_matrix("Zimbabwe"))/pop_i,Rowv = NA, Colv = NA)
heatmap(t(cs_ij_av),Rowv = NA, Colv = NA)

round(squire::get_mixing_matrix("Zimbabwe") - t(cs_ij_av*pop_i),3)
