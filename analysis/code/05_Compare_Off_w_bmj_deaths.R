...
fit <- fit_spline_rt(data = data,
                     ...
                     dur_get_ox_survive =durations_death_survive$Surv_Dur_Weighted,
                     dur_get_ox_die =durations_death_survive$Death_Dur_Weighted,
                     dur_R = Inf,
...
### BMJ data
df_bmj <- readRDS("analysis/data/Code-generated-data/00_05_BMJ_Data.rds")

# Approx est. based on 1 in 10 reporting, UTH has 80% coverage of deaths in Lusaka:
sum(df_bmj$TotEstCovDeathsLus_10x)

DeathTotalOff/sum(df_bmj$TotEstCovDeathsLus_10x)
...
df_off_fil_full <- readRDS("analysis/data/Code-generated-data/00_05b_BMJ_Data_DailyEsts.rds")
